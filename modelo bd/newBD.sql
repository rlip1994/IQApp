...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_addKid`(name varchar(50),age int(11),lastname1 varchar(50),lastname2 varchar(50), 
									idGrade int(11), idSchool int(11),zipCode varchar(15),detail1 varchar(300),
                                    detail2 varchar(300),idCity int(11),idCard varchar(20),
                                    favTopic varchar(50),favSubject varchar(50),
                                    favHobbie varchar(50),otherInfo varchar(100),idUser int)
BEGIN

 
INSERT INTO mydb.players(`name`,age,idgrade,lastname1,lastname2,enable,idCard,favoriteTopic,favoriteSubject,favoriteHobbie,otherInfo) VALUES (name,age,idGrade,lastName1,lastName2,1,idCard,favTopic,favSubject,favHobbie,otherInfo);
INSERT INTO mydb.addresses(zipcode,detail1,detail2,idCity) VALUES (zipCode,detail1,detail2,idCity);
INSERT INTO mydb.playersxschools(idplayer,idSchool)VALUES ((SELECT idplayer FROM mydb.players WHERE players.idCard = idCard),idSchool);
INSERT INTO mydb.playersxaddresses(idplayer,idAddress,`date`,enable) VALUES ((SELECT idplayer FROM mydb.players WHERE players.idCard = idCard),(SELECT idAddress FROM mydb.addresses ORDER BY idAddress DESC LIMIT 1),(SELECT CURDATE()),TRUE);
INSERT INTO mydb.playersxusers(idplayer,iduser) VALUES ((SELECT idplayer FROM mydb.players WHERE players.idCard = idCard),idUser);
...
SELECT players.idplayer,players.`name`,players.age,grades.`name` grade,
       ...
       cities.name city, addresses.detail1,addresses.detail2, addresses.zipCode, schools.name school FROM mydb.players 
       inner join mydb.grades on mydb.grades.idgrade=mydb.players.idgrade 
...
-- -----------------------------------------------------
-- procedure sp_getValidUser
-- -----------------------------------------------------

DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getValidUser`(id int, pass varchar(150))
BEGIN
select users.iduser from users where users.iduser = id and users.password = MD5(pass) limit 1;
END$$
...
UPDATE mydb.users SET users.username=username,users.email=email,users.`password`=MD5(pass),users.`name`=pname,
